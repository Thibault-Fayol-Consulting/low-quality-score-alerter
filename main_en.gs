/**
 * --------------------------------------------------------------------------
 * Low Quality Score Alerter — Google Ads Script
 * --------------------------------------------------------------------------
 * Identifies keywords with quality scores below a threshold using GAQL.
 * Reports creative quality, predicted CTR, and post-click breakdowns.
 * Sends alerts and optionally exports to Google Sheets.
 *
...
var CONFIG = {
  ...
  PAUSE_LOW_QS: false,
  SPREADSHEET_URL: '',
  SHEET_NAME: 'Low QS Keywords'
};

function main() {
  try {
    Logger.log('=== Low Quality Score Alerter ===');
    Logger.log('Threshold: QS < ' + CONFIG.MIN_QS);
    ...
    Logger.log('Low QS keywords: ' + lowQs.length);
    ...
    MailApp.sendEmail(CONFIG.EMAIL, 'Low QS Alerter — Error', e.message);
  }
}
...
function pauseKeywords_(keywords) {
  for (var i = 0; i < keywords.length; i++) {
    var kwIter = AdsApp.keywords()
      ...
      .get();
    if (kwIter.hasNext()) { kwIter.next().pause(); Logger.log('Paused: "' + keywords[i].text + '"'); }
  }
}

function sendAlert_(keywords) {
  var subject = (CONFIG.TEST_MODE ? '[TEST] ' : '') +
    'Low QS Alert — ' + keywords.length + ' keyword(s) below QS ' + CONFIG.MIN_QS;

  var body = 'Low Quality Score Report\n========================\n\n';
  body += 'Keywords with QS < ' + CONFIG.MIN_QS + ': ' + keywords.length + '\n\n';

  for (var i = 0; i < keywords.length; i++) {
    var kw = keywords[i];
    body += '- "' + kw.text + '" (QS: ' + kw.qs + ')\n';
    body += '  Creative: ' + kw.creative + ' | CTR: ' + kw.ctr + ' | Post-click: ' + kw.postClick + '\n';
    body += '  ' + kw.campaign + ' > ' + kw.adGroup + '\n\n';
  }

  body += 'Legend: ABOVE_AVERAGE / AVERAGE / BELOW_AVERAGE\n';
...
function exportToSheets_(keywords) {
  ...
  sheet.clear();
  sheet.appendRow(['Keyword', 'QS', 'Creative', 'Predicted CTR', 'Post-Click', 'Campaign', 'Ad Group']);
  for (var i = 0; i < keywords.length; i++) {
    var kw = keywords[i];
    sheet.appendRow([kw.text, kw.qs, kw.creative, kw.ctr, kw.postClick, kw.campaign, kw.adGroup]);
  }
  Logger.log('Exported ' + keywords.length + ' rows to Sheets.');
}
